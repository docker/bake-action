...
const githubModuleIds = new Set<string>(['@actions/github']);
try {
  githubModuleIds.add(require.resolve('@actions/github'));
} catch {
  // Ignore unresolved optional paths.
}
try {
  const toolkitRequire = createRequire(require.resolve('@docker/actions-toolkit/package.json'));
  githubModuleIds.add(toolkitRequire.resolve('@actions/github'));
} catch {
  // Ignore when toolkit or its transitive github dependency cannot be resolved.
}

for (const mod of githubModuleIds) {
  vi.doMock(mod, () => githubMock);
}
